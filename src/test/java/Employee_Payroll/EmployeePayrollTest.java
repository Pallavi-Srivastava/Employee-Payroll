...
public class EmployeePayrollTest {
	...
	public void givenEmployeePayroll_WhenAddNewRecord_ShouldeReturnUpdatedValue()
			throws EmployeePayrollException, SQLException {
		employeePayrollService.readEmployeePayrollData(IOService.DB_IO);
		employeePayrollService.addNewEmployeeToPayroll("Devin", 40000000.00, LocalDate.now(), "M", "Marketing", true);
		boolean result = employeePayrollService.checkUpdatedRecordSyncWithDatabase("Devin");
		Assert.assertTrue(result);
	}
	...
	public void givenEmployeePayroll_WhenDeleteRecord_ShouldSyncWithDB() throws EmployeePayrollException, SQLException {
		employeePayrollService.readEmployeePayrollData(IOService.DB_IO);
		employeePayrollService.deleteEmployeeToPayroll("Devin");
		boolean result = employeePayrollService.checkedRecordDeletedFromDatabase("Devin");
		Assert.assertTrue(result);
	}
}
